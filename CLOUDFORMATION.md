...
The CloudFormation is set up to upload the Lambda function code from a zip file in a public S3 bucket. The function is named ConnectGetVanityNumbers. You'll see in the AWS code editor that vanit-numbers-master folder (this entire repo) was placed in the root folder. You'll need to move the vanitynumbers folder and lambda_function into the root folder (ConnectGetVanityNumbers). Once they are in the root folder you can delete vanity-numbers-master folder.
